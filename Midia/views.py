...
def login(request):
    ...
    if request.method == "GET":
        ...
        usuario_autenticado = auth.authenticate(request, username=usuario.username, password=senha)
...
def criar_conta(request):
    if request.method == "GET":
        ...
        if User.objects.filter(email=email).exists() or User.objects.filter(username=email).exists():
            messages.add_message(request, constants.WARNING, "Já existe um usuário com esse email")

        else:
            try:
                User.objects.create_user(username=email, first_name=nome, perfil= PERFIS.USUARIO, email=email, password=senha)
                messages.add_message(request, constants.SUCCESS, "Conta criada com sucesso!")

            except Exception as erro:
                print(erro)
                messages.add_message(request, constants.ERROR, "Não foi possível criar conta")
                # return redirect("/")  
...
def buscar_filme(request):
    ...
    if request.method == "POST":
        ...
        medias = []
        properties_to_filter = ["title", "releaseDate", "voteAverage", "genres"]
        for media in json_data:
            if media["originalLanguage"] == "en":
                filtered_media = {key: media[key] for key in properties_to_filter if key in media}
